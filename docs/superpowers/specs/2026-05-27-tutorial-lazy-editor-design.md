...
### Section 1a — Typed filenames via codegen

Vite's `import.meta.glob` types the key as plain `string` regardless of the pattern, so template-literal inference like `keyof typeof sources extends \`./${infer Name}\`` resolves to `never`. To get autocomplete and compile-time validation on `snippet("…")` calls, we generate a `SnippetFilename` literal union at dev/build time.

Add a small Vite plugin at `site/vite-plugins/snippet-filenames.ts`:
...
function collectTsxFilenames(dir: string): string[] {
  const found: string[] = []
  for (const entry of readdirSync(dir)) {
    const full = join(dir, entry)
    const stat = statSync(full)
    if (stat.isDirectory()) {
      found.push(...collectTsxFilenames(full))
      continue
    }
    if (entry.endsWith(".tsx")) found.push(entry)
  }
  return found
}

export function snippetFilenamesPlugin(): Plugin {
  const snippetsDir = join(process.cwd(), "src/snippets")
  const outFile = join(snippetsDir, "filenames.gen.ts")

  function emit(): void {
    const filenames = collectTsxFilenames(snippetsDir).sort()
    const seen = new Set<string>()
    for (const name of filenames) {
      if (seen.has(name)) throw new Error(`duplicate snippet filename "${name}"`)
      seen.add(name)
    }
    const union = filenames.map(name => JSON.stringify(name)).join(" | ")
    const body = `// AUTO-GENERATED by site/vite-plugins/snippet-filenames.ts — do not edit.\nexport type SnippetFilename = ${union || "never"}\n`
    writeFileSync(outFile, body)
  }

  return {
    name: "solid-three:snippet-filenames",
    buildStart() {
      emit()
    },
    configureServer(server) {
      server.watcher.on("add", path => path.endsWith(".tsx") && path.includes("/snippets/") && emit())
      server.watcher.on("unlink", path => path.endsWith(".tsx") && path.includes("/snippets/") && emit())
    },
  }
}
```

Register it in `site/vite.config.ts` alongside the existing custom plugins. The generated file is committed to git (so CI typechecks work without running the plugin) and updated automatically on dev startup, build start, and any `.tsx` add/remove inside `snippets/`.

`site/src/snippets/index.ts` then imports the union and tightens the helper:

```ts
import type { SnippetFilename } from "./filenames.gen"

export function snippet(filename: SnippetFilename): SnippetEntry {
  // …
}
```

Per-call autocomplete works in MDX and TSX. Typos fail at typecheck rather than at runtime.
