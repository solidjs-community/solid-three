...
On the [`<Canvas/>`](/api/components/canvas), `onPointerMissed` is narrower: it fires only when the click hit **nothing at all** — the canvas-wide "clicked empty space" / deselect signal.
