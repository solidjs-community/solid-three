...
The engine also owns the raycaster it picks with — core has none. Configure it with `pointerEvents({ raycaster: … })`, and reach it at runtime with `useRaycaster()`. See [Raycasters](/api/utilities/raycasters).
