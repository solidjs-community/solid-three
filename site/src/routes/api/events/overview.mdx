...
Handlers are passed as props on any `<T.*>` or `<Entity/>` component. The same handler names are also accepted on `<Canvas/>`, where they fire after tree propagation finishes — useful for catching events that hit empty space (see [`onClickMissed`](#onclickmissed-and-friends-on-canvas)).
