...
and friends. [`<T.*>`](/api/components/t) already does this for
objects it creates from JSX; `autodispose` is the manual version for
objects you instantiate
yourself (e.g. to pass through [`<Entity from={...}/>`](/api/components/entity)).
...
- [`<Entity/>`](/api/components/entity) — the common reason to instantiate objects by hand and dispose them yourself.
- [`<T.*>`](/api/components/t) — already auto-disposes the objects it creates from JSX.
