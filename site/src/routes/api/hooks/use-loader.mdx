...
`useLoader` is built on top of `load`, a plain async function that wraps `loader.load` in a `Promise` (with no caching, no Suspense integration, no reactivity). Use it directly when you need to await a resource imperatively — e.g. inside an event handler, a one-shot setup effect, or non-component code.
...
`useLoader` calls `load` internally when there's a cache miss; reach for `load` when you want the same URL-normalisation and record-spread behaviour without going through Solid's reactive resource machinery.
