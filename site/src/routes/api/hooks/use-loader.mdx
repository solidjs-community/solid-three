...
`useLoader` loads asynchronous resources — textures, models, and the like — and exposes them through `solid-js`' reactivity. Use it inside `<Suspense>` to handle loading states. By default it caches every resource in a shared [`LoaderCache`](/api/utilities/loader-cache).
...
- [Loaders & Resource](/tutorial/06-loaders-and-resource) — the tutorial walk-through.
- [`LoaderCache`](/api/utilities/loader-cache) — the default global cache and its disposal helpers.
- [`<Resource/>`](/api/components/resource) — the component form, for loading straight into the scene graph.
