...
- **gl** (`Meta<ResolvedRenderer>`): The active renderer, wrapped with [`meta`](/api/utilities/metadata) so you can read solid-three metadata via [`getMeta(three.gl)`](/api/utilities/metadata). The underlying renderer is `WebGLRenderer | WebGPURenderer | RendererLike` by default — narrow project-wide via [Register augmentation](/api/components/canvas#narrowing-the-renderer-type-project-wide).
