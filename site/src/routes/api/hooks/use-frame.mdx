...
| `options` _(optional)_ | `{ priority?: number; stage?: "before" \| "after" }`         | `priority` orders callbacks — lower runs first (default `0`). `stage` runs the callback `"before"` (default) or `"after"` the render.                                    |
...
- [useFrame](/tour/05-use-frame) — the tutorial chapter.
