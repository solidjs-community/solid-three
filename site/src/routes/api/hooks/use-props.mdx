...
| `context` _(optional)_ | `Pick<Context, "requestRender" \| "gl" \| "props">` | The context slice `useProps` reads from. Defaults to [`useThree()`](/api/hooks/use-three). Pass it explicitly when calling outside a [`<Canvas>`](/api/components/canvas) — in tests, or a custom renderer host. |
...
| An event handler                             | `onClick={…}`                     | Registered on the [event system](/api/events/overview) (`Object3D` instances only).         |
...
- [`<T.*>` / `createT`](/api/components/t) and [`<Entity/>`](/api/components/entity) — the components built on `useProps`.
