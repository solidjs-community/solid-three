...
| `context` *(optional)* | `Pick<Context, "requestRender" \| "gl" \| "props">` | The context slice `useProps` reads from. Defaults to [`useThree()`](/api/hooks/use-three). Pass it explicitly when calling outside a [`<Canvas>`](/api/components/canvas) — in tests, or a custom renderer host. |
...
- [Props and children](/tutorial/02-props-and-children) — the tutorial walk-through of the same value conversion.
