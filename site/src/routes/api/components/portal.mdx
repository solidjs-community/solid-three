...
With no `element`, `Portal` attaches its children to `context.scene` — the root of the canvas's main scene. This is the escape hatch for "render _as if_ this child were top-level", ignoring any transform on the JSX parent.
...
- `Portal` doesn't break ownership. Children stay in the JSX tree for reactivity, refs, and lifecycle — only the _render destination_ changes.
...
- [Portal](/tour/07-portal) — the tutorial chapter, with a working render-to-texture demo.
