...
| ------------- | ------------------------------------------------------------------------------------ |
| `frameloop`   | `"always"`                                                                           |
| `gl`          | `new WebGLRenderer({ alpha: true, canvas })` (`alpha: true` is the implicit default — override it by passing the `[ctorArgs, properties]` tuple form, e.g. `gl={[{ alpha: false }, {}]}`) |
