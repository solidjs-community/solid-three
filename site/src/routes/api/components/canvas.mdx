...
| `raycaster`    | `Partial<Props<Raycaster>> \| Raycaster`                                                   | `new Raycaster()`                    | Configuration for a plain `THREE.Raycaster`. Core never casts this itself — it's config storage that a pointer-event engine (if one is installed) reads. See [Raycasters](/api/utilities/raycasters).                                       |
...
`onPointerMissed` is not a native `Canvas` prop — it's contributed by a pointer-event engine when one is listed on this canvas's `plugins` (typically via `createT.withCanvas`). See [Events overview](/api/events/overview#the-miss-onpointermissed).

<details>
<summary>Exact type</summary>

```tsx
interface CanvasProps extends ParentProps {
  ...
  class?: string
  // Plugins can contribute additional props here, e.g. onPointerMissed from pointerEvents()
}
```
...
By default [`useThree().gl`](/api/hooks/use-three) is typed as the open `SupportedRenderer` union. Declare your concrete renderer once and the type narrows everywhere — both `useThree().gl` reads and `<Canvas gl>` assignments:
