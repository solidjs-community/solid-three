...
| `onPointerMissed` | `Partial<CanvasEventHandlers>`                                                             | —                                    | Fires when a click hits empty space (a total miss) — the canvas-wide deselect signal. The only canvas-level 3D pointer handler; every other handler is object-level. See [events](/api/events/overview).                                    |
...
interface CanvasProps extends ParentProps<Partial<CanvasEventHandlers>> {
  ...
  class?: string
  // Plus onPointerMissed (Partial<CanvasEventHandlers>)
}
```
