...
[`<Canvas>`](/api/components/canvas) is already running an animation loop to render each frame.
...
that _new_ position and follows it (priority 1). Swap the priorities and
...
touch — see [`useFrame` in the API reference](/api/hooks/use-frame).

The scenes so far have been hand-built — cubes, lights, geometry from
constructors. The next chapter brings in _outside_ data: textures,
