...
`Resource` is the component form of [`useLoader`](/reference/hooks/use-loader) — it loads a resource and drops it straight into the scene graph, or hands it to a render function.
...
| `base` | `string` | Base URL for resolving relative paths (forwarded to [`useLoader`](/reference/hooks/use-loader)). |
| `cache` | `boolean \| LoaderRegistry` | `true` (default global cache), `false` (disable), or a custom registry. See [Caching](/reference/hooks/use-loader#caching). |
...
- [`useLoader`](/reference/hooks/use-loader) — the hook `Resource` wraps, with the full caching model.
