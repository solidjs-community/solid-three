...
- **In libraries** — create scoped `T` namespaces for tree-shaking, or use [`<Entity/>`](/reference/components/entity) instead.
- **Multiple namespaces** — create several `T` instances to lazy-load different parts of three.js.

## See also

- [`<Entity/>`](/reference/components/entity) — the namespace-free form, plus [`createEntity`](/reference/components/entity#createentity) for a single typed component.
- [`useProps`](/reference/hooks/use-props) — how props on every `<T.*>` component are applied.
