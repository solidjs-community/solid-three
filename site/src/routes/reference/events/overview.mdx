...
You attach handlers as props on any [`<T.*>`](/reference/components/t) or [`<Entity/>`](/reference/components/entity) component. The same handler names also work on [`<Canvas/>`](/reference/components/canvas), where they fire after tree propagation finishes — handy for catching clicks that land on empty space (see [Missed events](#missed-events)).
...
2. **Up the scene graph (tree)** — after an object's handler runs, the event bubbles up its ancestors. `stopPropagation()` stops this too. If nothing stops the event, it finally fires on the [`<Canvas/>`](/reference/components/canvas) itself.
...
The same handlers work on [`<Canvas/>`](/reference/components/canvas), where they fire when **no object handler at all** consumed the ray:
...
To keep an object from being hit while it still receives events that bubble up from its children, set `raycastable={false}`. See [`raycastable`](/reference/events/raycastable) for the full prop.

## See also

- [Pointer events](/tour/04-pointer-events) — the tutorial chapter, with a worked walk-through.
- [`raycastable`](/reference/events/raycastable) — opt an object out of hit-testing while keeping bubbled events.
- [`<Canvas/>`](/reference/components/canvas) — where canvas-level handlers and `onClickMissed` fire.
