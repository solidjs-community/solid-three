...
| `gl` | `Meta<ResolvedRenderer>` | The active renderer, wrapped with [`meta`](/reference/utilities/metadata) so you can read solid-three metadata via `getMeta(three.gl)`. Narrow it project-wide with [Register augmentation](/reference/components/canvas#narrowing-the-renderer-type-project-wide). |
...
| `scene` | `Meta<Scene>` | The root scene, wrapped with [`meta`](/reference/utilities/metadata). |
| `props` | `CanvasProps` | The props the host [`<Canvas>`](/reference/components/canvas) was rendered with. |
| `xr` | `{ connect: () => void; disconnect: () => void }` | WebXR connection management. |

## Behavior

The camera and raycaster are each managed as a **stack**. The `camera` and `raycaster` from [`<Canvas>`](/reference/components/canvas) props sit at the bottom; whatever is on top is the active one that `three.camera` / `three.raycaster` return.
...
- [`<Canvas/>`](/reference/components/canvas) — sets the default camera, raycaster, and scene this hook reads.
- [Raycasters](/reference/utilities/raycasters) — the raycaster types `setRaycaster` accepts.
- [Metadata](/reference/utilities/metadata) — reading solid-three metadata off `gl` and `scene`.
