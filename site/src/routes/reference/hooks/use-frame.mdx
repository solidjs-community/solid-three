...
| --- | --- | --- |
| `callback` | `(context: Context, delta: number, frame?: XRFrame) => void` | Runs each frame. `context` is the [three.js context](/reference/hooks/use-three), `delta` is seconds since the last frame, and `frame` is the `XRFrame` during WebXR sessions. |
...
- [`useThree`](/reference/hooks/use-three) — the `context` passed to every callback.
