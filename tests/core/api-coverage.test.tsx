...
describe("createEntity", () => {
  it("creates a component that constructs the given class with args", async () => {
    const Group = createEntity(THREE.Group)
    const { scene } = await test(() => <Group name="custom" />)
    expect(scene.children[0]).toBeInstanceOf(THREE.Group)
    expect(scene.children[0]?.name).toBe("custom")
  })

  it("passes args to the constructor", async () => {
...
describe("useProps", () => {
  it("reactively applies props to an existing instance", async () => {
...
describe("Resource", () => {
  it("adds an Object3D resource to the parent scene graph", async () => {
    const { scene } = await test(() => <Resource loader={MockObject3DLoader} url="model.glb" />)
  ...
  it("renders via children render function with the loaded resource", async () => {
    ...
    await test(() => (
      <Resource loader={MockLoader} url="render-fn.png">
        {resource => {
          // Solid 2's <Show> runs the render callback inside untrack, so calling
          // the `resource` accessor in plain JS triggers STRICT_READ_UNTRACKED.
          // This test only observes the URL once; untrack is the right pattern.
          observedUrl = (untrack(resource) as MockResource).url
          return null
        }}
      </Resource>
    ))
